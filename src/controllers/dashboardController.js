...
exports.getDashboardData = async (req, res) => {
  try {
    ...
    // Sum finalAmount from transactions that have no buyerId (anonymous/walk-in buyers)
    const withoutBuyerTotal = transactions
      .filter((t) => !t.buyerId)
      .reduce((sum, t) => sum + (t.finalAmount || 0), 0);

    const totalPayIn =
      buyerPayments.reduce((sum, p) => sum + (parseFloat(p.amount) || 0), 0) +
      withoutBuyerTotal;
    const totalPayOut = sellerPayments.reduce(
